...
// string is filepath, char is CST  C cli_fn  S serv_fn  T status_fn
std::map<std::string, std::pair<time_t, char> > syncing_map;
    ...
    for (i=0; i<nfds; ++i) {
      if (sockfd == events[i].data.fd) {  // 有新的连接
        ...
        if (num <= 0) {
          ...
          std::string mapkey = fdata.filename;
          syncing_map[mapkey] = std::make_pair(time(NULL), 'C');
          ...
          // 修改
            ...
            category.info("modify file %s ", fdata.filename);
            // 修改文件
            ...
            if (fdata.end_mark == true) {  //  文件传输结束
              if ((it = fileopen_map.find(filename)) != fileopen_map.end()) {
                ...
                int fd = open(realpath.c_str(), O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
                if (fd != -1) {
                  close(fd);
                  printf("BBBB\n");
                }
          ...
          // 新建
            ...
            // 新建文件
              ...
              category.info("create file %s success", fdata.filename);
            }
          }
          else {
...
void *serv_fn(void *arg) {
  ...
  while(1) {
    ...
    while(i < len) {
      ...
      std::map<std::string, std::pair<time_t, char> >::iterator mit = syncing_map.find(filename);
      if (mit != syncing_map.end()) {
        if ((time(NULL) - mit->second.first < 10) && mit->second.second == 'C') {
          category.info("synchronizing, not transport data.");
          continue;
        }
      }
      syncing_map[filename] = std::make_pair(time(NULL), 'S');
      ...
      printf("===before trans data\n");
	  trans_data(servfd, filename.c_str(), operate, is_dir, rootpath);
      printf("===after trans data\n");
      close(servfd);
    }
  }  
}
    ...
    for (i=0; i<nfds; ++i) {
      if (sockfd == events[i].data.fd) {  // 有新的连接
        ...
        if (num <= 0) {
          ...
          // 判断文件是否正在传输
          std::string filename = fresult.filename;
          std::map<std::string, std::pair<time_t, char> >::iterator mit = syncing_map.find(filename);
          if (mit != syncing_map.end()) {
            if ((time(NULL) - mit->second.first < 10) && mit->second.second == 'C') {
              category.info("synchronizing, not transport data.");
          ...
          syncing_map[filename] = std::make_pair(time(NULL), 'T');
		  ...
		  if (memcmp(md5value, fresult.md5value, 32) != 0) {
			...
			trans_data(servfd, fresult.filename, 'U', false, rootpath);
			close(servfd);
		  }
		  else {
